...
@Component({
  ...
  standalone: true,
  imports: [
    MatCardModule,MatCardHeader,ReactiveFormsModule,RouterLink
  ],
  ...
  processRegisteration() {
    ...
    if (this.registerForm.valid) {
      this.auth.processRegisteration(this.registerForm.value).subscribe(
         (res) => {
          this._coreService.openSnackBar('please contact admin for complete process','Registration successful!');
        //  this.route.navigateByUrl('login')
        },
        (error) => {
          const errorMsg = error.error.errors[0]?.msg || 'An error occurred during registration.';
